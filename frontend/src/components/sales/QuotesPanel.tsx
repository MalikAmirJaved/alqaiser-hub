...
export default function QuotesPanel() {
  ...
  const computeKPIs = (data: Quote[]) => {
    ...
    return [
      ...
      },
      {
        ...
        sub: `${acceptedCount} quotes accepted`,
        tone: "success" as const,
        isCurrency: true,
      },
      {
        label: "Win Rate",
        value: data.length > 0 ? `${((acceptedCount / data.length) * 100).toFixed(1)}%` : "0%",
        sub: "Accepted ratio",
  ...
  return (
    <DynamicModulePage
      ...
      onCreate={() => router.push("/sales/quotes/new")}
      actions={{
        onEdit: (quote) => router.push(`/sales/quotes/${quote.id}`),
