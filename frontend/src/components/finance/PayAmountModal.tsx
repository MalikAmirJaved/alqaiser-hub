...
export default function PayAmountModal({
  ...
  const isValidAmount =
    !isNaN(parsedAmount) && parsedAmount > 0 && parsedAmount <= outstandingNum + 0.001;

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isValidAmount) {
      setError(`Enter an amount between 0.01 and ${formatCurrency(outstandingNum)}`);
      return;
    }
    setError("");
    await onSubmit({
  ...
  const setQuickAmount = (val: number) => {
    const clamped = Math.min(Math.max(val, 0), outstandingNum);
    setAmount(clamped > 0 ? String(clamped) : "");
    setError("");
  };

  const remainingAfter = isValidAmount ? Math.max(0, outstandingNum - parsedAmount) : outstandingNum;
  const newStatus =
    !isValidAmount
      ? paymentStatus
      : remainingAfter <= 0.001
        ? "PAID"
        : parsedAmount < outstandingNum
          ? "PARTIAL"
          : "PAID";
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="rounded-xl border border-primary/30 bg-primary/5 px-3 py-2.5 text-center">
              ...
              <p className="text-sm font-bold font-mono mt-0.5 text-primary">{formatCurrency(outstandingNum)}</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              {outstandingNum > 2 && (
                <button
                  type="button"
                  onClick={() => setQuickAmount(Math.round(outstandingNum / 2 * 100) / 100)}
          ...
          <div className="flex gap-2 pt-1">
            ...
            >
              {isPending ? "Processing…" : `Pay ${isValidAmount ? formatCurrency(parsedAmount) : ""}`}
            </button>
          </div>
        </form>
      </div>
