...
import { useRelatedTabData, type RelatedTab, type PaginatedResult } from "@/hooks/useProductRelatedData";
...
import { ArrowUpRight, User, Building2, Hash, Tag, Barcode, DollarSign, Package, Warehouse, Eye, ChevronLeft, ChevronRight } from "lucide-react";
...
function PaginationControls({
  ...
  setPagination: React.Dispatch<React.SetStateAction<TabPagination>>;
}) {
  if (!data || data.total_pages <= 1) return null;
  const start = (data.current_page - 1) * 20 + 1;
  const end = Math.min(data.current_page * 20, data.count);
  return (
    <div className="flex items-center justify-between pt-3 border-t border-border/40">
      <p className="text-xs text-muted-foreground">
        {data.count === 0 ? "No results" : `${start}–${end} of ${data.count}`}
      </p>
...
function TabFilterBar({
  ...
  return (
    <div className="flex items-center gap-2 mb-3">
      <div className="w-56">
        <DateRangePickerRac
          startDate={pagination.dateFrom}
          endDate={pagination.dateTo}
          onChange={(start, end) =>
            setPagination(p => ({ ...p, page: 1, dateFrom: start, dateTo: end }))
          }
...
function StockMovementsTab({ productId }: { productId: string }) {
  const [pagination, setPagination] = useState<TabPagination>({ page: 1 });
  const { data, isLoading } = useRelatedTabData({
    productId,
    tab: "stock-movements",
  ...
  if (items.length === 0) return (
    <>
      <TabFilterBar pagination={pagination} setPagination={setPagination} />
      <div className="py-12 text-center text-sm text-muted-foreground">No stock movements found</div>
    </>
  );
  ...
  return (
    <>
      ...
      <PaginationControls data={data} pagination={pagination} setPagination={setPagination} />
    </>
  );
}

function PurchaseOrdersTab({ productId }: { productId: string }) {
...
function SalesOrdersTab({ productId }: { productId: string }) {
  const formatCurrency = useFormatCurrency();
  const [pagination, setPagination] = useState<TabPagination>({ page: 1 });
  const { data, isLoading } = useRelatedTabData({
    productId,
  ...
  if (items.length === 0) return (
    <>
      <TabFilterBar pagination={pagination} setPagination={setPagination} />
      <div className="py-12 text-center text-sm text-muted-foreground">No sales orders found</div>
    </>
  );
...
function QuotesTab({ productId }: { productId: string }) {
  const router = useRouter();
  const formatCurrency = useFormatCurrency();
  const [pagination, setPagination] = useState<TabPagination>({ page: 1 });
  const { data, isLoading } = useRelatedTabData({
  ...
  if (items.length === 0) return (
    <>
      <TabFilterBar pagination={pagination} setPagination={setPagination} />
      <div className="py-12 text-center text-sm text-muted-foreground">No quotes found</div>
    </>
  );
...
function InvoicesTab({ productId }: { productId: string }) {
  const router = useRouter();
  const formatCurrency = useFormatCurrency();
  const [pagination, setPagination] = useState<TabPagination>({ page: 1 });
  const { data, isLoading } = useRelatedTabData({
      ...
      <DetailLayout
        ...
        tabs={[
          ...
          },
          {
            ...
            render: () => <StockMovementsTab productId={id} />,
          },
          {
            id: "purchase-orders",
            label: "Purchase Orders",
            render: () => <PurchaseOrdersTab productId={id} />,
          },
          {
            id: "sales-orders",
            label: "Sales Orders",
            render: () => <SalesOrdersTab productId={id} />,
          },
          {
            id: "quotes",
            label: "Quotes",
            render: () => <QuotesTab productId={id} />,
          },
          {
            id: "invoices",
            label: "Invoices",
            render: () => <InvoicesTab productId={id} />,
          },
        ]}
        sidebar={
          <StandardSidebar
